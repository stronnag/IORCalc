...
void calc_rig(ior_rec_t *u, calc_rec_t *c) {
  ...
  c->tci = (u->fs > 0.0) ? MAX(0.0, u->fbi - u->fs + 0.04 * u->b) : 0.0;

  if (u->go + u->mw == 0.0) {
       c->i = u->ig + c->tci;
    c->sph = u->sps + c->tci;
    c->psh = MAX(0.0, c->sph - 0.25 * c->i);
    c->bad = u->bas + c->tci;
  } else {
       c->bad = u->bas - 0.04 * u->b + c->tci;
       c->i = MAX(u->isp, u->ig + u->ig * (u->go - u->mw) / (u->j - u->go + u->mw)) -
        0.04 * u->b + c->tci;
    c->psh = MAX(0.0, u->sps - 0.27 * u->isp);
  }

  c->jc = MAX(u->j, MAX(u->spl, u->smw / 1.8));
  c->shbs = MAX(0.0, u->hbs - 0.05 * c->jc);
  c->plf = MAX(0.0, 2.0 * (u->sl - 0.95 * sqrt(c->i * c->i + c->jc * c->jc)));
  c->tti = c->plf + c->psh + c->shbs * 1.8 * u->sl / MAX(1.0, u->smw);
  c->ic = c->i + c->tti;
  c->lp = MAX(1.5 * c->jc, MAX(u->lpg + u->fsp, u->lpis));

  c->smwm = trunc_to(MAX(u->smw, 1.8 * c->jc), u->munit + 1);
  c->slm = trunc_to(MAX(u->sl, 0.95 * sqrt(c->i * c->i + c->jc * c->jc)), u->munit + 1);
  c->ajlmx = sqrt(u->j * u->j + u->ig * u->ig);

  c->rsaf = 0.5 * c->ic * c->jc * (1.0 + (1.1 * (c->lp - c->jc) / c->lp)) +
         MAX(0.0, 0.125 * c->jc * (c->ic - 2 * c->jc));

  c->ec = u->e + MAX(u->bal - CONSTANTS[9][u->munit], 0.0);
  ...
  c->xbl1 = c->xbl2 = MAX(0.21 * u->e, CONSTANTS[4][u->munit] + 0.1 * u->e);
  c->xbl3 = c->xbl4 = MAX(0.34 * u->e, CONSTANTS[4][u->munit] + 0.12 * u->e);

  c->xblp = 0.2 * u->p;
  c->xhb = MAX(CONSTANTS[11][u->munit], 0.04 * u->e);

  c->p1 = MAX(0.0, u->bd - 0.05 * u->e);
  c->p2 = MAX(0.0, u->hb - c->xhb) * u->p / u->e;
  c->p3 = MAX(0.0, c->xblp - u->blp);
  c->p4 = MAX(0.0, u->bl1 + c->p3 - c->xbl1);
  c->p5 = MAX(0.0, u->bl2 - c->xbl2) / 6.0;
  c->p6 = MAX(0.0, u->bl3 - c->xbl3) / 6.0;
  c->p7 = MAX(0.0, u->bl4 - c->xbl4) / 6.0;

  c->p8 = (u->e > 40.0 / CONSTANTS[6][u->munit]) ? MAX(0.0, u->bl5 - c->xbl2) / 6.0 : 0.0;

  c->xbl1 = MAX(u->bl1, c->xbl1 - c->p3);
  c->xbl2 = MAX(u->bl2, c->xbl2);
  c->xbl3 = MAX(u->bl3, c->xbl3);
  c->xbl4 = MAX(u->bl4, c->xbl4);

  c->xhb = MAX(u->hb, c->xhb);

  /*   if (u->g == 0.0)
        {
  */
  c->bpen = (c->p4 + c->p5 + c->p6 + c->p7 + c->p8) * u->p / u->e;
  c->p9 = MAX(0.0, c->bad - 0.05 * u->p - CONSTANTS[10][u->munit] - c->tci);
  c->p10 = MAX(0.0, 0.96 * c->rh - u->p - u->bas);

  c->xmgu = MAX(0.28 * u->e + 0.016 * u->p + CONSTANTS[15][u->munit], 0.38 * u->e);
  c->xmgm = MAX(0.5 * u->e + 0.022 * u->p + CONSTANTS[16][u->munit], 0.65 * u->e);

  if (u->mgu * u->mgm > 0.0) {
    c->gpen = u->p / u->e * (MAX(0.0, u->mgu - c->xmgu) + MAX(0.0, u->mgm - c->xmgm));
    c->rgf = (u->hb + 2.0 * u->mgu + 3.0 * u->mgm + 2.0 * u->e) /
          (u->hb + 2 * c->xmgu + 3 * c->xmgm + 2 * u->e);

    c->rgf = MAX(c->rgf, 0.83);
    c->rgf = MIN(1.0, c->rgf);
  ...
  c->pc = u->p + c->ttp;
  c->rsam = 0.35 * c->pc * c->ec + MAX(0.0, 0.2 * c->ec * (c->pc - 2 * u->e));
  /*
        }
     else
        {
  ...
  */
  ...
  c->rsama = c->rsam;
  c->rsam = MAX(c->smin, c->rsam * c->rgf);
  ...
  */
  ...
  c->satca = MAX(0.3 * (c->rsaf - 2.2 * c->rsam), -0.25 * c->rsaf);
  c->rsat =c-> rsam + c->rsaf + c->satc;
  c->rsata = MIN(c->rsaf + c->rsam + c->satca, c->rsat);
  /*
        }
     else
   if is > 0 then
  ...
  */
  c->rsal = MAX(1.3 * c->rsam, c->rsal);
  c->spin = 1.01 * c->jc * MAX(0.95 * sqrt(c->i * c->i + c->jc * c->jc), u->sl);
  c->rsala = c->rsal;
  c->s = sqrt(MAX(c->rsat, MAX(c->rsal, c->spin)));
  c->shr = c->s * (8.66 / c->l + 1.0 / sqrt(u->b * c->mdia)) - c->l / (10 *CONSTANTS[0][u->munit]);
  c->scf = MAX(1.0, 1 + 0.04 * (c->shr - 16.1));
  c->sc = c->s * c->scf;
}
