...
void calc_rig(ior_rec_t *u, calc_rec_t *c) {
    ...
    if (u->go + u->mw == 0.0) {
        ...
        c->i = fmax(u->isp,
                    u->ig + u->ig * (u->go - u->mw) / (u->j - u->go + u->mw)) -
               0.04 * u->b + c->tci;
    ...
    c->plf =
        fmax(0.0, 2.0 * (u->sl - 0.95 * sqrt(c->i * c->i + c->jc * c->jc)));
    ...
    c->slm = trunc_to(fmax(u->sl, 0.95 * sqrt(c->i * c->i + c->jc * c->jc)),
                      u->munit + 1);
    ...
    c->p8 = (u->e > 40.0 / CONSTANTS[6][u->munit])
                ? fmax(0.0, u->bl5 - c->xbl2) / 6.0
                : 0.0;
    ...
    */
    ...
    c->xmgu =
        fmax(0.28 * u->e + 0.016 * u->p + CONSTANTS[15][u->munit], 0.38 * u->e);
    c->xmgm =
        fmax(0.5 * u->e + 0.022 * u->p + CONSTANTS[16][u->munit], 0.65 * u->e);

    if (u->mgu * u->mgm > 0.0) {
        c->gpen = u->p / u->e *
                  (fmax(0.0, u->mgu - c->xmgu) + fmax(0.0, u->mgm - c->xmgm));
    ...
    c->pc = u->p + c->ttp;
    c->rsam =
        0.35 * c->pc * c->ec + fmax(0.0, 0.2 * c->ec * (c->pc - 2 * u->e));
    /*
          }
       else
          {
    ...
    */
    ...
    c->spin =
        1.01 * c->jc * fmax(0.95 * sqrt(c->i * c->i + c->jc * c->jc), u->sl);
    c->rsala = c->rsal;
    c->s = sqrt(fmax(c->rsat, fmax(c->rsal, c->spin)));
    c->shr = c->s * (8.66 / c->l + 1.0 / sqrt(u->b * c->mdia)) -
             c->l / (10 * CONSTANTS[0][u->munit]);
