...
void calc_hull(ior_rec_t *u, calc_rec_t *c) {
    ...
    c->mdia = (u->mdias > 0.0)
                  ? u->mdias + (c->omdi / u->b) * (u->bwl + 0.75 * u->b) / 2.0
    ...
    if (u->fbms == 0.0 && u->fbm != 0.0)
        u->fbms =
            trunc_to(u->fbm + c->sink - c->trim * (u->sbmax - u->fgo) / c->lbg,
                     u->munit + 2);
    ...
    c->pd = (u->pds != 0.0)
                ? u->pds + c->sink - c->trim * (u->spd - u->fgo) / c->lbg
                : 0.0;
    ...
    c->esd =
        (u->esds != 0.0)
            ? u->esds + c->sink - c->trim * (u->spd - u->esl - u->fgo) / c->lbg
            : 0.0;

    if (u->sfs != 0.0 && u->fss == 0.0 && u->fs != 0.0) {
        u->fss = u->fs + c->sink - c->trim * (u->sfs - u->fgo) / c->lbg;
        u->fbis =
            u->fbi + c->sink - c->trim * (u->sfs + u->j - u->fgo) / c->lbg;
    ...
    if (u->fss != 0.0) {
        ...
        u->fbi =
            u->fbis - c->sink + (u->sfs + u->j - u->fgo) * c->trim / c->lbg;
    }

    if (c->ihy > 1979)
        u->fs =
            fmin(u->fs, c->ff + (c->ff - c->ffi) * (u->fgo - u->sfs) / u->gsdf);
    ...
    if (c->ihy > 1986) {
        ...
        c->cffi =
            c->ff - c->ffi +
            (4 * c->fmd - c->fa - 3 * c->ff) * u->gsdf / c->lbg +
    ...
    fbot = 0.125 * u->b + c->ff - c->cffi - c->ffi - 0.15 * (u->bfi - u->bf) +
           0.5 * u->gdfi;
    ...
    c->foc =
        fmin(c->foc, 1.5 * u->gsdf * 0.25 * u->b / (0.25 * u->b + u->gdfi));
    ...
    c->fdic = (c->fdi > 0.435 * z) ? fmin(0.475 * z, 0.2175 * z + .5 * c->fdi)
                                   : c->fdi;
    ...
    if (u->gsda == 0.0)
        ...
        c->agsl = u->gsda /
                  (c->fa - c->fai - 0.2 * (u->bai - u->ba) + c->hgli - c->hgla);
        ...
        c->apslc = (c->fa - u->vha - c->ccor[4] - 0.018 * c->lbgc < 0.0)
                       ? fmin(c->apslc, 6.0)
                       : fmin(c->apslc, 4.0);
        ...
        c->ycor =
            (c->ihy > 1980)
                ? fmax(1.7 * u->vha - 0.5 * u->gd - u->y, 0.15 * c->lbg - u->y)
        ...
        c->aoc =
            fmin(0.5 * (c->aocp + c->aocg), 0.05 * c->lbg + 0.95 * c->aocp);
        c->aoc = c->ycor + fmin(c->aoc, 0.05 * c->lbg + 0.95 * c->aocg);

        c->aocc = (c->aoc + u->y < 0.0) ? fmin(.6 * c->aoc, -u->y)
                                        : fmin(c->aoc, 1.25 * u->gsda);
    }
    ...
    c->bdr = pow((2.165 * pow(c->l * CONSTANTS[6][u->munit], 0.525) -
                  CONSTANTS[19][u->munit]),
                 0.375) /
    ...
    if (u->cd == 0.0) {
        ...
        c->rd = pow(pow(u->cd, 1.6) +
                        pow(fmax(c->dm - u->dmc - 0.3 * c->db, 0.0), 1.6),
                    0.625) +
                fmin(0.3 * c->db, c->dm);
