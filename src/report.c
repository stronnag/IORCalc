    ...
    *buf = 0;
    ...
    if (u->bha)
        t1 = sqrt(pow(0.5 * u->ba - 0.15 * u->b, 2.0) + u->bha * u->bha) +
             sqrt(0.0225 * u->ba * u->ba +
                  (u->vha - u->bha) * (u->vha - u->bha));
    else
        t1 = sqrt(u->vha * u->vha + 0.25 * u->ba * u->ba);

    show_error(&buf, "Error in aft girth, gla %8.3g, calc %8.3g", 2 * t1,
               0.75 * u->b + u->gd);

    t2 = 0.049 * CONSTANTS[4][u->munit];

    if (u->dmt) {
        t1 = u->fmds + (u->fais - u->fmds) * (u->sdm - u->fgo - 0.5 * c->lbg) /
                           (0.5 * c->lbg - u->gsda);
        show_error(&buf, "DMT approximation error %8.4g",
                   fabs(u->dmt - u->dms - t1), t2);
    }
    /*
      if (u->pdt) {
           t1 = u->fmds +
                (u->fais - u->fmds) * (u->spd - u->fgo - 0.5 * c->lbg) / (0.5 *
      c->lbg - u->gsda); show_error(&buf, "PDT approximation error %8.4g",
      fabs(u->pdt - u->pds - t1), t2);
      }
    */
    if (u->glai) {
        t1 = 0.875 * u->b + u->gd;
    ...
    show_error(&buf, "Depth correction to PF", c->pf,
               PFACTOR[u->ipt - 1][c->ecod - 1]);

    if (u->st1) {
        if (u->apt) {
            show_error(&buf, "Strut Fail 1.5*prdc > edl", 1.5 * c->prdc,
                       u->apt);
            show_error(&buf, " Strut Fail edc > 0.75*prdc", u->apb,
                       0.75 * c->prdc);
            show_error(&buf, " Strut Fail 0.08*prdc > st1", 0.08 * c->prdc,
                       u->st1);
            show_error(&buf, " Strut Fail 0.3*prdc > st2", 0.3 * c->prdc,
                       u->st2);
            show_error(&buf, " Strut Fail st3 > 0.75*prdc", 0.75 * c->prdc,
                       u->st3);
        } else {
            show_error(&buf, " Exp Shaft Fail 1.5*prdc > esl", 1.5 * c->prdc,
                       u->esl);
            show_error(&buf, " Exp Shaft Fail esc > 0.75*prdc", u->esc,
                       0.75 * c->prdc);
            show_error(&buf, " Exp Shaft Fail 0.05*prdg > st1", 0.05 * c->prdc,
                       u->st1);
            show_error(&buf, " Exp Shaft Fail 0.2*prdc > st2", 0.2 * c->prdc,
                       u->st2);
            show_error(&buf, " Exp Shaft Fail st3 > 0.5*prdc", u->st3,
                       0.5 * c->prdc);
        }
    }

    show_error(&buf, "LPG+FSP %8.3g exceeds 1.5*JC %8.3g", u->lpg + u->fsp,
               1.5 * c->jc);

    if (c->ic > c->i) {
        show_error(&buf, "SPS %8.3g exceeds 0.27*ISP %8.3g", u->sps,
                   0.27 * u->isp);
        show_error(&buf, "HBS %8.3g exceeds 0.05*JC %8.3g", u->hbs,
                   0.05 * c->jc);
    }

    show_error(&buf, "BAL %8.3g exceeds limit %8.3g", u->bal,
               CONSTANTS[9][u->munit]);
