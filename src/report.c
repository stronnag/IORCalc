...
char *report(ior_rec_t *u, calc_rec_t *c) {
  ...
  *buf = 0;
  ...
  t2 = 0.049 * get_constant(u->munit, 4);

  if (u->dmt) {
    t1 = u->fmds + (u->fais - u->fmds) * (u->sdm - u->fgo - 0.5 * c->lbg) / (0.5 * c->lbg - u->gsda);
  ...
  */
  ...
  show_error(&buf, "Depth correction to PF", c->pf, get_pfactor(u->ipt - 1, c->ecod - 1));
  ...
  show_error(&buf, "BAL %8.3g exceeds limit %8.3g", u->bal, get_constant(u->munit, 9));

  if (c->pc > u->p) {
    show_error(&buf, "BAS %8.3g exceeds limit %8.3g", c->bad, 0.05 * u->p + c->tci + get_constant(u->munit, 10));
