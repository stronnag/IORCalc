...
void calc_rmc(ior_rec_t *u, calc_rec_t *c) {
    ...
    c->tr = fmax(5.15, CONSTANTS[8][u->munit] * 0.97 * c->l * pow(u->bwl, 3.0) /
                           c->rmc);
    c->cgf = (c->tr > 35.0)
                 ? 0.0064 * c->tr + 0.744
                 : 2.2 / (c->tr - 5.1) +
                       0.00023 * c->l * CONSTANTS[6][u->munit] + 0.8693;
    ...
    c->bwl1 = fmin(3.0 + (u->omd - c->fmd) / (u->md - u->omd) * u->b / 4.0,
                   0.8 * u->b);

    if (c->bwl1 < u->bwl)
        c->bwl1 = u->bwl;

    c->sv = ((0.03 * c->l * CONSTANTS[12][u->munit] * pow(c->bwl1, 3.0) -
              54.0 * c->rm) /
                 c->dspl +
