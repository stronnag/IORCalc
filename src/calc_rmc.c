...
void calc_rmc(ior_rec_t *u, calc_rec_t *c) {
  ...
  c->tr = fmax(5.15, (get_constant(u->munit, 8) * 0.97 * c->l * pow(u->bwl, 3.0) / c->rmc));

  c->cgf = (c->tr > 35.0) ? 0.0064 * c->tr + 0.744 : 2.2 / (c->tr - 5.1) + 0.00023 * c->l * get_constant(u->munit, 6) + 0.8693;

  if (c->cgf > 0.968 || c->ihy > 1984 || (c->ihy == 1984 && c->imo > 10))
    c->cgfm = 0.968;
  else {
    c->cgfm = 0.000115 * c->l * get_constant(u->munit, 6) + 0.9588;
  ...
  c->sv = ((0.03 * c->l * get_constant(u->munit, 12) * pow(c->bwl1, 3.0) - 54.0 * c->rm) / c->dspl + 0.6 * c->cmdi -
           0.54 * u->cmd) *
              get_constant(u->munit, 6) +
          0.25;
}
