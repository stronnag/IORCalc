...
void calc_prop(ior_rec_t *u, calc_rec_t *c) {
  ...
  while (1) {
    ...
    c->em = u->ew * u->ewd;
    c->emf = get_constant(u->munit, 7) * c->em / (c->l * c->l * u->b * c->d);
    ...
    if (u->aph != 0.0) {
      c->ecod = (u->aph < u->prd * 1.125 || fmin(u->apt, u->apb) < 0.4 * u->prd) ? INAPTS : INAPT;
      break;
    }
  ...
  if (u->prd > 0.0) {
    ...
    c->pf = get_pfactor(ipt - 1, c->ecod - 1) - pfc;

    c->df = (c->ps < 0.03 * c->l)
                ? c->pf * sqrt(c->pdc / c->db) * (fmax(c->prdc * c->prdc * 60.0 / (c->l * c->l), 0.024) - 0.024)
