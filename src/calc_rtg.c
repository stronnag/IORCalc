...
void calc_rtg(ior_rec_t *u, calc_rec_t *c) {
    ...
    if (idiv < 2) {
        ...
        c->cgfc = fmin(
            1.0, fmax(0.5, 0.375 * u->b / (c->cmdi + c->mdi + c->omdi) - 0.5));
        ...
        if (idiv == 3) {
            ...
            c->shra =
                sqrt(c->msar) * (8.66 / c->l + 1.0 / sqrt(u->b * c->mdia)) -
    ...
    c->mr = (0.13 * c->l * c->sc / sqrt(u->b * c->d) + 0.25 * c->l +
             0.2 * c->sc + c->dc + c->fc) *
            c->dlf;
    ...
    if (c->ihy > 1986) {
        c->csf =
            pow(((1.98 * (0.2 * (u->bmax + 1.5 * u->bai) * u->bmax / u->b -
                          0.085 * (fbx - c->fai))) /
    ...
    c->mra1 = (0.13 * c->l * c->sca / sqrt(u->b * c->d) + 0.25 * c->l +
               0.2 * c->sca + c->fc + c->dc) *
              c->dlfa;
    pars = c->sca * (0.0777 * c->sca / sqrt(u->b * c->d) + 0.2216);
    parl = c->l *
           (0.0659 * c->l / pow((c->l * u->b * c->mdia), 1.0 / 3.0) + 0.1738);
    ...
    c->ra = fmax(c->rior * 0.85, c->mra * c->epf * c->cgfa * c->maf * c->cbfa *
                                     c->smf * c->lrp * c->tpf);
    if (c->ihy > 1981)
    ...
    c->w =
        ...
        (CONSTANTS[6][u->munit] * CONSTANTS[6][u->munit] * c->l * u->b * u->b +
         (u->p - c->d / 2.0) * (5.0 * CONSTANTS[6][u->munit] * u->p - 110.0) +
         35.0 * c->r) /
        (CONSTANTS[14][u->munit] * (c->i + 0.67 * c->d));
    c->crewno = (int)(0.4 * c->r * CONSTANTS[6][u->munit] + 0.02) - 2;
    c->crewwt = (int)((74.0 * c->r * CONSTANTS[6][u->munit] - 370.0) /
                      CONSTANTS[14][u->munit]);
